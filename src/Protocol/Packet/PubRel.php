...
/**
 * PUBREL packet model for MQTT 3.1.1 and 5.0.
 *
 * The PUBREL packet is sent by the sender in response to a PUBREC packet with QoS level 2.
 * It is the third packet in the QoS 2 four-packet handshake, releasing the message for delivery.
 *
 ...
 * 3. Sender sends PUBREL packet to release the message ← This packet
 * 4. Receiver sends PUBCOMP packet to confirm completion
 *
 * Key Differences Between MQTT Versions:
 * - MQTT 3.1.1: Simple packet with Packet Identifier only
 * - MQTT 5.0: Adds Reason Code and Properties for enhanced error reporting
 *
 * MQTT 3.1.1 Structure:
 * - Fixed Header: Type (6), flags (0x02), Remaining Length (2)
 ...
 *
 * Important Note:
 * - PUBREL is the only QoS 2 packet that requires specific flags (0x02) in the fixed header
 * - Bits 3-0 of the fixed header first byte must be 0b0010
 *
 * MQTT 5.0 Reason Codes:
 * - 0x00 (0): Success - Message released
...
final class PubRel
{
    ...
    /**
     * @param  int  $packetId  Packet identifier matching the PUBREC packet (1-65535)
     * @param  int  $reasonCode  MQTT 5.0 reason code (0 = success, 0x92 = not found). Always 0 for MQTT 3.1.1.
     * @param  array<string, mixed>|null  $properties  MQTT 5.0 PUBREL properties. Possible keys:
    ...
    /**
     * Check if the packet indicates success.
    ...
    /**
     * Check if the packet indicates an error.
    ...
    /**
     * Get a property value from MQTT 5.0 PUBREL properties.
     *
    ...
    /**
     * Check if a property exists in MQTT 5.0 PUBREL properties.
     */
     ...
     * Provides additional human-readable information about the result.
     */
